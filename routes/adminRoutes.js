...
router.get(
  ...
  wrapAsync(async (req, res) => {
    const { user } = req;
    const pending_coaches = await Coach.find({ status: "pending" }).populate(
      "team"
    );
    const approved_coaches = await Coach.find({ status: "approved" }).populate(
      "team"
    );
    const disqualified_coaches = await Coach.find({
      status: "disqualified",
    }).populate("team");
...
router.post(
  ...
  wrapAsync(async (req, res) => {
    ...
    qrcode.toDataURL(
      `http://localhost:3000/${team._id}/player/show`,
...
router.delete(
  "/admin/team/:teamId/delete",
  wrapAsync(async (req, res) => {
    const { teamId } = req.params;
    const team = await Team.findByIdAndDelete(teamId);

    if (team) {
      await Coach.updateMany({ team: teamId }, { $unset: { team: "" } });
      await Student.updateMany({ team: teamId }, { $unset: { team: "" } });

      if (team.image && team.image.filename) {
        await uploader.destroy(team.image.filename);
      }

      if (team.audio && team.audio.filename) {
        await uploader.destroy(team.audio.filename);
      }

      if (team.teamImage && team.teamImage.filename) {
...
router.get(
  ...
  wrapAsync(async (req, res) => {
    ...
    if (!students.length) {
      req.flash(
        "error",
        "Sorry, no data was found matching your selected filters. Please adjust your filters and try again."
      );
