  ...
  passport.authenticate("coach", (err, user, info) => {
    ...
    req.logIn(user, async (err) => {
      ...
      if (user instanceof Coach && user.status === "approved") {
        ...
        const payment_pending = coach.students.filter(
          (student) =>
            student.registrationMode === "bulk" &&
        ...
        req.flash(
          "error",
          `${user.fullname} your application is not yet approved. Please wait until it's verified.`
        );
        return res.redirect(`/coach/login`);
      }
    });
...
router.get(
  ...
  wrapAsync(async (req, res) => {
    ...
    const team = await Team.findOne({ coaches: { $in: [id] } })
      .populate("students")
      .populate("coaches");
    const footballers = team.students.filter(
      (student) => student.role === "football" && student.dop === dopNum
    );
    const cheerleaders = team.students.filter(
      (student) => student.role === "cheer" && student.dop === dopNum
    );
    const approved_students = team.students.filter(
      (student) => student.status === "approved" && student.dop === dopNum
    );
    const pending_students = team.students.filter(
      (student) => student.status === "pending" && student.dop === dopNum
    );
    const disqualified_students = team.students.filter(
      (student) => student.status === "disqualified" && student.dop === dopNum
    );
...
router.get(
  "/team/:teamId/division/:division/jersey/:jerseyNumber/check",
  wrapAsync(async (req, res) => {
    const { teamId, division, jerseyNumber } = req.params;
    const student = await Student.findOne({
      team: teamId,
      dop: division,
      jersey: jerseyNumber.toUpperCase(),
    });
...
router.get(
  ...
  wrapAsync(async (req, res, next) => {
    ...
    const coach = await Coach.findById(user._id)
      .populate("students")
      .populate("team");
    const bulk_students = coach.students.filter(
      (student) =>
        student.registrationMode === "bulk" &&
        student.paymentStatus === "unpaid"
    );
  ...
  wrapAsync(async (req, res) => {
    ...
    req.files.forEach((file) => {
      ...
      if (match) {
        ...
        if (type === "image" || type === "captureImage") {
          ...
          const documentName = Array.isArray(req.body.documentType[playerIndex])
            ? req.body.documentType[playerIndex][documentIndex]
            : req.body.documentType[playerIndex];
    ...
    for (const playerIndex in playerFiles) {
      ...
      const student = new Student({
        ...
        image:
          images.length > 0
            ? { filename: images[0].filename, path: images[0].path }
            : null,
...
router.post(
  ...
  wrapAsync(async (req, res, next) => {
    ...
    const pending_payment = coach.students.filter(
      (student) =>
        student.paymentStatus === "unpaid" &&
        student.registrationMode === "bulk"
    );
...
router.get(
  ...
  wrapAsync(async (req, res) => {
    ...
    res.render("./coach/studentEdit", { student, docName });
  })
);

router.put(
  "/coach/student/profile/:id",
  wrapAsync(async (req, res) => {
    const { id } = req.params;
    await Student.findByIdAndUpdate(id, { ...req.body });
    req.flash("success", "Information has been updated successfully.");
    res.redirect(`/coach/student/edit/${id}`);
  })
);
